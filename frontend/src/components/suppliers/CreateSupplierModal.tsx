...
import { CreateSupplierRequest, PersonType, Supplier } from '@/types/supplier';
...
interface CreateSupplierModalProps {
  ...
  onDuplicateFound: (supplier: Supplier, document: string, documentType: 'CPF' | 'CNPJ') => void;
}
...
export const CreateSupplierModal: React.FC<CreateSupplierModalProps> = ({
  ...
  const checkDocumentDuplicate = async (document: string, personType: PersonType) => {
    ...
    try {
      // Buscar todos os fornecedores para verificar duplicatas localmente
      const allSuppliers = await suppliersAPI.getSuppliers({ limit: 1000 });
      
      const existingSupplier = allSuppliers.find(supplier => 
        supplier.document.replace(/\D/g, '') === cleanDoc
      );
      
      if (existingSupplier) {
        const docType = personType === PersonType.PF ? 'CPF' : 'CNPJ';
        onDuplicateFound(existingSupplier, document, docType);
      }
    } catch (error) {
