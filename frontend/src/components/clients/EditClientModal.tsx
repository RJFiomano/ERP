...
import { Client, UpdateClientRequest, PersonType } from '@/types/client';
...
export const EditClientModal: React.FC<EditClientModalProps> = ({
  ...
  useEffect(() => {
    if (client) {
      setFormData({
        ...
        contacts: {
          phones: client.phones ? client.phones.map(phone => ({
            number: phone.number,
            type: phone.type,
          ...
          addresses: client.addresses ? client.addresses.map(address => ({
            type: address.type,
  ...
  const handleChange = (field: keyof UpdateClientRequest, value: string | PersonType) => {
            ...
            <Grid item xs={12}>
              <Box display="flex" alignItems="center" justifyContent="space-between">
                <Typography variant="body2" color="text.secondary">
                  Cliente ativo: {client.is_active ? 'Sim' : 'Não'}
                </Typography>
